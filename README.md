...
**测试环境：** AMD Ryzen 5 5600G (6C12T, 3.9 GHz) / 16 GB RAM / Zenoh TCP 回环 (127.0.0.1)
...
| 测试 | 平均耗时 | 中位数 | OPS |
|------|---------|--------|-----|
| 小消息单向 (50B) | **88.8 µs** | 78.0 µs | 11,266 ops/s |
| 大消息单向 (4KB) | **140.8 µs** | 132.0 µs | 7,100 ops/s |
...
| 测试 | 平均耗时 | 中位数 | OPS |
|------|---------|--------|-----|
| RPC 往返调用 (Add) | **30.8 µs** | 25.2 µs | 32,462 ops/s |
...
|------|---------|----------------------|----------------|
| VGA JPEG (640×480) | **227.5 KB** | 171 µs / 153 µs | **9,659** |
| HD JPEG (1280×720) | **680.4 KB** | 1,066 µs / 1,003 µs | **842** |
| Full HD JPEG (1920×1080) | **1,533.1 KB** | 3,466 µs / 3,416 µs | **317** |
| VGA RAW (640×480, RGB) | **900.0 KB** | — | **588** |
...
|------|--------|----------------------|------|
| 消息编码 (2字段) | **0.31 µs** | ~1-3 µs¹ | **3-10x** 快 |
| 消息解码 (2字段) | **0.36 µs** | ~1-3 µs¹ | **3-8x** 快 |
| Pub/Sub 单向 (50B) | **88.8 µs** | ~300-2000 µs² | **3-23x** 快 |
| Pub/Sub 单向 (4KB) | **140.8 µs** | ~500-5000 µs² | **4-36x** 快 |
| RPC 往返调用 | **30.8 µs** | ~500-3000 µs² | **16-97x** 快 |
| 图像 VGA JPEG 传输 | **171 µs** | ~4-10 ms³ | **23-58x** 快 |
| 图像 HD JPEG 传输 | **1,066 µs** | ~8-20 ms³ | **8-19x** 快 |
| 图像 FHD JPEG 传输 | **3,466 µs** | ~15-50 ms³ | **4-14x** 快 |
